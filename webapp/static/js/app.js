...
function initializeEventListeners() {
    ...
    document.getElementById('target-component').addEventListener('change', (e) => {
        state.targetComponent = e.target.value;
        if (state.targetComponent) {
            addComponentToSelection(state.targetComponent);
        }
    });
...
function renderParameters() {
    ...
    if (state.selectedComponents.length >= 2) {
        ...
        for (let i = 0; i < state.selectedComponents.length - 1; i++) {
            const from = state.selectedComponents[i];
            const to = state.selectedComponents[i + 1];
            edgesToShow.push({ from, to });
        }
    } else {
